...
public final class LabelDriver {
  ...
  public void dumpVectors() throws IOException {
    ...
    SequenceFileVectorWriter vectorWriter = getSeqFileWriter(outFile);
    try {
      long numDocs = vectorWriter.writeAndLabel(iterable, maxDocs);
  ...
  private static SequenceFileVectorWriter getSeqFileWriter(String outFile) throws IOException {
    Path path = new Path(outFile);
    Configuration conf = new Configuration();
    FileSystem fs = FileSystem.get(conf);
    // TODO: Make this parameter driven

    SequenceFile.Writer seqWriter = SequenceFile.createWriter(fs, conf, path, Text.class,
